...
parser = argparse.ArgumentParser(description="Implementation of downstream SSL MICA")
...
if __name__ == "__main__" :
    ...
    print(args)
    ssl_path = f'{PATH + args.weight_dir}/ssl_{int(100 * args.percent_data)}.pth'
    list_img_path = []
    ...
    app = FaceAnalysis( providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
    app.prepare(ctx_id=5, det_size= target_size)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with open( f'{PATH + args.weight_dir}/train_ssl_{int(100*args.percent_data)}_loss.pkl', 'wb') as file:
            dump(loss_train_his, file)
        loss_val_his.append(cur_val_loss)
        
        with open( f'{PATH + args.weight_dir}/val_ssl_{int(100*args.percent_data)}_loss.pkl', 'wb') as file:
        ...
        if min_val_loss > cur_val_loss:
            ...
            torch.save(model_dict,  f'{PATH + args.weight_dir}/train_ssl_{int(100*args.percent_data)}.pth.tar')
            
#         scheduler.step()


    save_path =   f'{PATH + args.weight_dir}/train_ssl_{int(100*args.percent_data)}.pth.tar'
